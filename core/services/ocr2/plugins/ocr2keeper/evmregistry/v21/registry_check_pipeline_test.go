...
import (
	...
	mocks2 "github.com/smartcontractkit/chainlink/v2/core/services/ocr2/plugins/ocr2keeper/evmregistry/v21/mocks"
...
func TestRegistry_VerifyCheckBlock(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			state:       encoding2.CheckBlockInvalid,
			retryable:   false,
			makeEthCall: true,
		},
		{
...
func TestRegistry_VerifyLogExists(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			reason:      encoding2.UpkeepFailureReasonNone,
			state:       encoding2.RpcFlakyFailure,
			retryable:   true,
			makeEthCall: true,
		...
		},
		{
			...
			reason:      encoding2.UpkeepFailureReasonTxHashNoLongerExists,
			retryable:   false,
			makeEthCall: true,
			blocks: map[int64]string{
		...
		},
		{
			...
			reason:    encoding2.UpkeepFailureReasonTxHashReorged,
			retryable: false,
			blocks: map[int64]string{
				500: "0xa518faeadcc423338c62572da84dda35fe44b34f521ce88f6081b703b250cca4",
			},
...
func TestRegistry_CheckUpkeeps(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			results: []ocr2keepers.CheckResult{
				{
					PipelineExecutionState: uint8(encoding2.CheckBlockInvalid),
					Retryable:              false,
					Eligible:               false,
				...
				{
					PipelineExecutionState: uint8(encoding2.RpcFlakyFailure),
					Retryable:              true,
					Eligible:               false,
					IneligibilityReason:    0,
					UpkeepID:               uid1,
				...
				{
					PipelineExecutionState: uint8(encoding2.PackUnpackDecodeFailed),
					Retryable:              false,
					Eligible:               false,
					IneligibilityReason:    0,
					UpkeepID:               uid2,
...
func TestRegistry_SimulatePerformUpkeeps(t *testing.T) {
	...
	cr0 := ocr2keepers.CheckResult{
		PipelineExecutionState: uint8(encoding2.CheckBlockTooOld),
		Retryable:              false,
		Eligible:               false,
		IneligibilityReason:    0,
		UpkeepID:               uid0,
...
func setupEVMRegistry(t *testing.T) *EvmRegistry {
	...
	mockReg := mocks2.NewRegistry(t)
	mockHttpClient := mocks2.NewHttpClient(t)
