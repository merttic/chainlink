...
import (
	...
	mercury2 "github.com/smartcontractkit/chainlink/v2/core/services/ocr2/plugins/ocr2keeper/evmregistry/v21/mercury"
	"github.com/smartcontractkit/chainlink/v2/core/services/ocr2/plugins/ocr2keeper/evmregistry/v21/mercury/v02"
	"github.com/smartcontractkit/chainlink/v2/core/services/ocr2/plugins/ocr2keeper/evmregistry/v21/mercury/v03"
...
type streams struct {
	...
	packer          mercury2.Packer
	mercuryConfig   mercury2.MercuryConfigProvider
	abi             abi.ABI
	...
	v02Client       mercury2.MercuryClient
	v03Client       mercury2.MercuryClient
}
...
func (s *streams) Lookup(ctx context.Context, checkResults []ocr2keepers.CheckResult) []ocr2keepers.CheckResult {
	lookups := map[int]*mercury2.StreamsLookup{}
	...
	for i, lookup := range lookups {
		wg.Add(1)
		func(i int, lookup *mercury2.StreamsLookup) {
...
func (s *streams) buildResult(ctx context.Context, i int, checkResult ocr2keepers.CheckResult, checkResults []ocr2keepers.CheckResult, lookups map[int]*mercury2.StreamsLookup) {
	lookupLggr := s.lggr.With("where", "StreamsLookup")
	if checkResult.IneligibilityReason != uint8(mercury2.MercuryUpkeepFailureReasonTargetCheckReverted) {
	...
	streamsLookupResponse := &mercury2.StreamsLookup{StreamsLookupError: streamsLookupErr}

	if len(streamsLookupResponse.Feeds) == 0 {
		checkResults[i].IneligibilityReason = uint8(mercury2.MercuryUpkeepFailureReasonInvalidRevertDataInput)
	...
	if streamsLookupResponse.IsMercuryV02() {
		...
		if state, reason, retryable, allowed, err := s.allowedToUseMercury(opts, upkeepId.BigInt()); err != nil {
			...
			checkResults[i].IneligibilityReason = uint8(mercury2.MercuryUpkeepFailureReasonMercuryAccessNotAllowed)
			return
		}
...
func (s *streams) doLookup(ctx context.Context, wg *sync.WaitGroup, lookup *mercury2.StreamsLookup, i int, checkResults []ocr2keepers.CheckResult) {
	defer wg.Done()

	state, reason, values, retryable, retryInterval, err := mercury2.NoPipelineError, mercury2.MercuryUpkeepFailureReasonInvalidRevertDataInput, [][]byte{}, false, 0*time.Second, fmt.Errorf("invalid revert data input: feed param key %s, time param key %s, feeds %s", lookup.FeedParamKey, lookup.TimeParamKey, lookup.Feeds)
	...
	if failureReason == uint8(mercury2.MercuryUpkeepFailureReasonMercuryCallbackReverted) {
		checkResults[i].IneligibilityReason = uint8(mercury2.MercuryUpkeepFailureReasonMercuryCallbackReverted)
		s.lggr.Debugf("at block %d upkeep %s requested time %s mercury callback reverts", lookup.Block, lookup.UpkeepId, lookup.Time)
		return
	}

	if !needed {
		checkResults[i].IneligibilityReason = uint8(mercury2.MercuryUpkeepFailureReasonUpkeepNotNeeded)
	...
	checkResults[i].IneligibilityReason = uint8(mercury2.MercuryUpkeepFailureReasonNone)
...
func (s *streams) allowedToUseMercury(opts *bind.CallOpts, upkeepId *big.Int) (state mercury2.MercuryUpkeepState, reason mercury2.MercuryUpkeepFailureReason, retryable bool, allow bool, err error) {
	allowed, ok := s.mercuryConfig.IsUpkeepAllowed(upkeepId.String())
	if ok {
		return mercury2.NoPipelineError, mercury2.MercuryUpkeepFailureReasonNone, false, allowed.(bool), nil
	}
	...
	if err != nil {
		...
		return mercury2.PackUnpackDecodeFailed, mercury2.MercuryUpkeepFailureReasonNone, false, false, fmt.Errorf("failed to pack upkeepId: %w", err)
	}
	...
	if err = s.client.CallContext(opts.Context, &resultBytes, "eth_call", args, hexutil.EncodeBig(opts.BlockNumber)); err != nil {
		return mercury2.RpcFlakyFailure, mercury2.MercuryUpkeepFailureReasonNone, true, false, fmt.Errorf("failed to get upkeep privilege config: %v", err)
	}

	var upkeepPrivilegeConfigBytes []byte
	upkeepPrivilegeConfigBytes, err = s.packer.UnpackGetUpkeepPrivilegeConfig(resultBytes)
	if err != nil {
		return mercury2.PackUnpackDecodeFailed, mercury2.MercuryUpkeepFailureReasonNone, false, false, fmt.Errorf("failed to get upkeep privilege config: %v", err)
	}

	if len(upkeepPrivilegeConfigBytes) == 0 {
		s.mercuryConfig.SetUpkeepAllowed(upkeepId.String(), false, cache.DefaultExpiration)
		return mercury2.NoPipelineError, mercury2.MercuryUpkeepFailureReasonMercuryAccessNotAllowed, false, false, fmt.Errorf("upkeep privilege config is empty")
	}

	var privilegeConfig UpkeepPrivilegeConfig
	if err = json.Unmarshal(upkeepPrivilegeConfigBytes, &privilegeConfig); err != nil {
		return mercury2.MercuryUnmarshalError, mercury2.MercuryUpkeepFailureReasonNone, false, false, fmt.Errorf("failed to unmarshal privilege config: %v", err)
	}

	s.mercuryConfig.SetUpkeepAllowed(upkeepId.String(), privilegeConfig.MercuryEnabled, cache.DefaultExpiration)

	return mercury2.NoPipelineError, mercury2.MercuryUpkeepFailureReasonNone, false, privilegeConfig.MercuryEnabled, nil
}

func (s *streams) checkCallback(ctx context.Context, values [][]byte, lookup *mercury2.StreamsLookup) (mercury2.MercuryUpkeepState, bool, hexutil.Bytes, error) {
	payload, err := s.abi.Pack("checkCallback", lookup.UpkeepId, values, lookup.ExtraData)
	if err != nil {
		return mercury2.PackUnpackDecodeFailed, false, nil, err
	}
	...
	if err := s.client.CallContext(ctx, &b, "eth_call", args, hexutil.EncodeUint64(lookup.Block)); err != nil {
		return mercury2.RpcFlakyFailure, true, nil, err
	}

	return mercury2.NoPipelineError, false, b, nil
}
