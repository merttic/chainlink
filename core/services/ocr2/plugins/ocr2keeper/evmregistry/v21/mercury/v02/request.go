	...
	for i := 0; i < resultLen; i++ {
		...
		if m.Error != nil {
			...
			if m.State != mercury2.NoPipelineError {
				state = mercury2.MercuryUpkeepState(m.State)
			}
			continue
		}
...
func (c *client) singleFeedRequest(ctx context.Context, ch chan<- mercury2.MercuryData, index int, sl *mercury2.StreamsLookup) {
	...
	if err != nil {
		ch <- mercury2.MercuryData{Index: index, Error: err, Retryable: false, State: mercury2.InvalidMercuryRequest}
		return
	}

	ts := time.Now().UTC().UnixMilli()
	signature := mercury2.GenerateHMACFn(http.MethodGet, mercuryPathV02+q.Encode(), []byte{}, c.mercuryConfig.Credentials().Username, c.mercuryConfig.Credentials().Password, ts)
