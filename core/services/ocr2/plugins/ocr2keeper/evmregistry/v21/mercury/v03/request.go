...
type client struct {
	...
	httpClient    mercury2.HttpClient
	threadCtrl    utils.ThreadControl
	lggr          logger.Logger
}

func NewClient(mercuryConfig mercury2.MercuryConfigProvider, httpClient mercury2.HttpClient, threadCtrl utils.ThreadControl, lggr logger.Logger) *client {
	return &client{
...
func (c *client) DoRequest(ctx context.Context, streamsLookup *mercury2.StreamsLookup, pluginRetryKey string) (mercury2.MercuryUpkeepState, mercury2.MercuryUpkeepFailureReason, [][]byte, bool, time.Duration, error) {
	resultLen := len(streamsLookup.Feeds)
	ch := make(chan mercury2.MercuryData, resultLen)
	if len(streamsLookup.Feeds) == 0 {
		return mercury2.NoPipelineError, mercury2.MercuryUpkeepFailureReasonInvalidRevertDataInput, [][]byte{}, false, 0 * time.Second, fmt.Errorf("invalid revert data input: feed param key %s, time param key %s, feeds %s", streamsLookup.FeedParamKey, streamsLookup.TimeParamKey, streamsLookup.Feeds)
	}
	resultLen = 1
	...
	retryable := true
	allSuccess := true
	state := mercury2.NoPipelineError
	...
	for i := 0; i < resultLen; i++ {
		...
		if m.Error != nil {
			...
			allSuccess = false
			if m.State != mercury2.NoPipelineError {
				state = m.State
			}
			continue
		}
	...
	if retryable && !allSuccess {
		retryInterval = mercury2.CalculateRetryConfigFn(pluginRetryKey, c.mercuryConfig)
	}
	// only retry when not all successful AND none are not retryable
	return state, mercury2.MercuryUpkeepFailureReasonNone, results, retryable && !allSuccess, retryInterval, reqErr
}

func (c *client) multiFeedsRequest(ctx context.Context, ch chan<- mercury2.MercuryData, sl *mercury2.StreamsLookup) {
	...
	params := fmt.Sprintf("%s=%s&%s=%s", mercury2.FeedIDs, strings.Join(sl.Feeds, ","), mercury2.Timestamp, sl.Time.String())
	...
	if err != nil {
		ch <- mercury2.MercuryData{Index: 0, Error: err, Retryable: false, State: mercury2.InvalidMercuryRequest}
		return
	}

	ts := time.Now().UTC().UnixMilli()
	signature := mercury2.GenerateHMACFn(http.MethodGet, mercuryBatchPathV03+params, []byte{}, c.mercuryConfig.Credentials().Username, c.mercuryConfig.Credentials().Password, ts)
	...
	retryErr := retry.Do(
		func() error {
			...
			if err != nil {
				...
				retryable = true
				state = mercury2.MercuryFlakyFailure
				return err
			}
			...
			if err != nil {
				retryable = false
				state = mercury2.InvalidMercuryResponse
				return err
			}
			...
			case http.StatusUnauthorized:
				retryable = false
				state = mercury2.UpkeepNotAuthorized
			...
			case http.StatusBadRequest:
				retryable = false
				state = mercury2.InvalidMercuryRequest
			...
			if err := json.Unmarshal(body, &response); err != nil {
				...
				retryable = false
				state = mercury2.MercuryUnmarshalError
				return err
			}
